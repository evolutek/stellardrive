...
package body BMP390_SPI is

   --  BMP390 SPI read: clock (reg or 16#80#), then the chip returns 1 dummy
   --  byte before the data. With Transmit_Receive the incoming bytes are:
   --  (1) during the address phase, (2) the dummy, (3..) the actual data.
   ...
   procedure read_byte
     (device   : in out BMP390_Device;
      ...
      value    : out UInt8)
   is
      tx : constant UInt8_Buffer (1 .. 3) := (register or 16#80#, 0, 0);
      rx : UInt8_Buffer (1 .. 3);
   begin
      Clear (device.cs);
      device.port.Transmit_Receive (tx, rx, rx'Length);
      Set (device.cs);
      value := rx (3);
   end read_byte;
   ...
   procedure write_byte
     (device   : in out BMP390_Device;
      ...
      value    : UInt8)
   is
      tx : constant UInt8_Buffer (1 .. 2) := (register and 16#7F#, value);
      rx : UInt8_Buffer (1 .. 2);
   begin
      Clear (device.cs);
      device.port.Transmit_Receive (tx, rx, rx'Length);
   ...
   procedure read_bytes
     (device   : in out BMP390_Device;
      register : UInt8;
      buffer   : out UInt8_Buffer)
   is
      n  : constant Natural := buffer'Length;
      tx : UInt8_Buffer (1 .. n + 2) := (1 => register or 16#80#, others => 0);
      rx : UInt8_Buffer (1 .. n + 2);
   begin
      Clear (device.cs);
      device.port.Transmit_Receive (tx, rx, rx'Length);
      Set (device.cs);
   ...
   is
      ...
      data : UInt8_Buffer(1..21) := (others => 0);
   begin
