...
--  BNO085 IMU over SPI (mode 3). SHTP protocol. The SPI port is configured by
--  the caller; this driver drives CS/RST and watches INT (HINTN, active low).
package BNO085_SPI is
  ...
  type BNO085_Device is record
      ...
      int_pin     : STM32.GPIO.GPIO_Point;   -- HINTN, active low (data ready)
      rst_pin     : STM32.GPIO.GPIO_Point;   -- active-low reset
      wake_pin    : STM32.GPIO.GPIO_Point;   -- PS0/WAKE, active low (host wants to send)
      initialized : boolean;               -- reset-complete announcement seen
      streaming   : boolean := False;       -- at least one sensor report parsed
