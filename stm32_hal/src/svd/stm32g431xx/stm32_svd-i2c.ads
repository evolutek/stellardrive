...
package STM32_SVD.I2C is
   ...
   subtype CR1_PE_Field is Boolean;
   subtype CR1_TXIE_Field is Boolean;
   subtype CR1_RXIE_Field is Boolean;
   subtype CR1_ADDRIE_Field is Boolean;
   subtype CR1_NACKIE_Field is Boolean;
   subtype CR1_STOPIE_Field is Boolean;
   subtype CR1_TCIE_Field is Boolean;
   subtype CR1_ERRIE_Field is Boolean;
   subtype CR1_DNF_Field is HAL.UInt4;
   subtype CR1_ANFOFF_Field is Boolean;
   subtype CR1_TXDMAEN_Field is Boolean;
   subtype CR1_RXDMAEN_Field is Boolean;
   subtype CR1_SBC_Field is Boolean;
   subtype CR1_NOSTRETCH_Field is Boolean;
   subtype CR1_WUPEN_Field is Boolean;
   subtype CR1_GCEN_Field is Boolean;
   subtype CR1_SMBHEN_Field is Boolean;
   subtype CR1_SMBDEN_Field is Boolean;
   subtype CR1_ALERTEN_Field is Boolean;
   subtype CR1_PECEN_Field is Boolean;

   --  Control register 1
   type CR1_Register is record
      --  Peripheral enable
      PE             : CR1_PE_Field := False;
      --  TX Interrupt enable
      TXIE           : CR1_TXIE_Field := False;
      --  RX Interrupt enable
      RXIE           : CR1_RXIE_Field := False;
      --  Address match interrupt enable (slave only)
      ADDRIE         : CR1_ADDRIE_Field := False;
      --  Not acknowledge received interrupt enable
      NACKIE         : CR1_NACKIE_Field := False;
      --  STOP detection Interrupt enable
      STOPIE         : CR1_STOPIE_Field := False;
      --  Transfer Complete interrupt enable
      TCIE           : CR1_TCIE_Field := False;
      --  Error interrupts enable
      ERRIE          : CR1_ERRIE_Field := False;
      --  Digital noise filter
      DNF            : CR1_DNF_Field := 16#0#;
      --  Analog noise filter OFF
      ANFOFF         : CR1_ANFOFF_Field := False;
      --  unspecified
      Reserved_13_13 : Boolean := False;
      --  DMA transmission requests enable
      TXDMAEN        : CR1_TXDMAEN_Field := False;
      --  DMA reception requests enable
      RXDMAEN        : CR1_RXDMAEN_Field := False;
      --  Slave byte control
      SBC            : CR1_SBC_Field := False;
      --  Clock stretching disable
      NOSTRETCH      : CR1_NOSTRETCH_Field := False;
      --  Wakeup from STOP enable
      WUPEN          : CR1_WUPEN_Field := False;
      --  General call enable
      GCEN           : CR1_GCEN_Field := False;
      --  SMBus Host address enable
      SMBHEN         : CR1_SMBHEN_Field := False;
      --  SMBus Device Default address enable
      SMBDEN         : CR1_SMBDEN_Field := False;
      --  SMBUS alert enable
      ALERTEN        : CR1_ALERTEN_Field := False;
      --  PEC enable
      PECEN          : CR1_PECEN_Field := False;
      --  unspecified
   ...
   subtype CR2_RD_WRN_Field is Boolean;
   subtype CR2_ADD10_Field is Boolean;
   subtype CR2_HEAD10R_Field is Boolean;
   subtype CR2_START_Field is Boolean;
   subtype CR2_STOP_Field is Boolean;
   subtype CR2_NACK_Field is Boolean;
   subtype CR2_NBYTES_Field is HAL.UInt8;
   subtype CR2_RELOAD_Field is Boolean;
   subtype CR2_AUTOEND_Field is Boolean;
   subtype CR2_PECBYTE_Field is Boolean;
   ...
   type CR2_Register is record
      ...
      RD_WRN         : CR2_RD_WRN_Field := False;
      --  10-bit addressing mode (master mode)
      ADD10          : CR2_ADD10_Field := False;
      --  10-bit address header only read direction (master receiver mode)
      HEAD10R        : CR2_HEAD10R_Field := False;
      --  Start generation
      START          : CR2_START_Field := False;
      --  Stop generation (master mode)
      STOP           : CR2_STOP_Field := False;
      --  NACK generation (slave mode)
      NACK           : CR2_NACK_Field := False;
      --  Number of bytes
      NBYTES         : CR2_NBYTES_Field := 16#0#;
      --  NBYTES reload mode
      RELOAD         : CR2_RELOAD_Field := False;
      --  Automatic end mode (master mode)
      AUTOEND        : CR2_AUTOEND_Field := False;
      --  Packet error checking byte
      PECBYTE        : CR2_PECBYTE_Field := False;
      --  unspecified
   ...
   subtype OAR1_OA1MODE_Field is Boolean;
   subtype OAR1_OA1EN_Field is Boolean;
   ...
   type OAR1_Register is record
      ...
      OA1MODE        : OAR1_OA1MODE_Field := False;
      --  unspecified
      Reserved_11_14 : HAL.UInt4 := 16#0#;
      --  Own Address 1 enable
      OA1EN          : OAR1_OA1EN_Field := False;
      --  unspecified
   ...
   subtype OAR2_OA2EN_Field is Boolean;
   ...
   subtype TIMEOUTR_TIDLE_Field is Boolean;
   subtype TIMEOUTR_TIMOUTEN_Field is Boolean;
   subtype TIMEOUTR_TIMEOUTB_Field is HAL.UInt12;
   subtype TIMEOUTR_TEXTEN_Field is Boolean;
   ...
   type TIMEOUTR_Register is record
      ...
      TIDLE          : TIMEOUTR_TIDLE_Field := False;
      --  unspecified
      Reserved_13_14 : HAL.UInt2 := 16#0#;
      --  Clock timeout enable
      TIMOUTEN       : TIMEOUTR_TIMOUTEN_Field := False;
      --  Bus timeout B
      TIMEOUTB       : TIMEOUTR_TIMEOUTB_Field := 16#0#;
      --  unspecified
      Reserved_28_30 : HAL.UInt3 := 16#0#;
      --  Extended clock timeout enable
      TEXTEN         : TIMEOUTR_TEXTEN_Field := False;
   end record
     with Volatile_Full_Access, Object_Size => 32,
          Bit_Order => System.Low_Order_First;
   ...
   subtype ISR_TXIS_Field is Boolean;
   subtype ISR_RXNE_Field is Boolean;
   subtype ISR_ADDR_Field is Boolean;
   subtype ISR_NACKF_Field is Boolean;
   subtype ISR_STOPF_Field is Boolean;
   subtype ISR_TC_Field is Boolean;
   subtype ISR_TCR_Field is Boolean;
   subtype ISR_BERR_Field is Boolean;
   subtype ISR_ARLO_Field is Boolean;
   subtype ISR_OVR_Field is Boolean;
   subtype ISR_PECERR_Field is Boolean;
   subtype ISR_TIMEOUT_Field is Boolean;
   subtype ISR_ALERT_Field is Boolean;
   subtype ISR_BUSY_Field is Boolean;
   subtype ISR_DIR_Field is Boolean;
   subtype ISR_ADDCODE_Field is HAL.UInt7;

   --  Interrupt and Status register
   type ISR_Register is record
      --  Transmit data register empty (transmitters)
      TXE            : ISR_TXE_Field := True;
      --  Transmit interrupt status (transmitters)
      TXIS           : ISR_TXIS_Field := False;
      --  Read-only. Receive data register not empty (receivers)
      RXNE           : ISR_RXNE_Field := False;
      --  Read-only. Address matched (slave mode)
      ADDR           : ISR_ADDR_Field := False;
      --  Read-only. Not acknowledge received flag
      NACKF          : ISR_NACKF_Field := False;
      --  Read-only. Stop detection flag
      STOPF          : ISR_STOPF_Field := False;
      --  Read-only. Transfer Complete (master mode)
      TC             : ISR_TC_Field := False;
      --  Read-only. Transfer Complete Reload
      TCR            : ISR_TCR_Field := False;
      --  Read-only. Bus error
      BERR           : ISR_BERR_Field := False;
      --  Read-only. Arbitration lost
      ARLO           : ISR_ARLO_Field := False;
      --  Read-only. Overrun/Underrun (slave mode)
      OVR            : ISR_OVR_Field := False;
      --  Read-only. PEC Error in reception
      PECERR         : ISR_PECERR_Field := False;
      --  Read-only. Timeout or t_low detection flag
      TIMEOUT        : ISR_TIMEOUT_Field := False;
      --  Read-only. SMBus alert
      ALERT          : ISR_ALERT_Field := False;
      --  unspecified
      Reserved_14_14 : Boolean := False;
      --  Read-only. Bus busy
      BUSY           : ISR_BUSY_Field := False;
      --  Read-only. Transfer direction (Slave mode)
      DIR            : ISR_DIR_Field := False;
   ...
   subtype ICR_ADDRCF_Field is Boolean;
   subtype ICR_NACKCF_Field is Boolean;
   subtype ICR_STOPCF_Field is Boolean;
   subtype ICR_BERRCF_Field is Boolean;
   subtype ICR_ARLOCF_Field is Boolean;
   subtype ICR_OVRCF_Field is Boolean;
   subtype ICR_PECCF_Field is Boolean;
   subtype ICR_TIMOUTCF_Field is Boolean;
   subtype ICR_ALERTCF_Field is Boolean;
